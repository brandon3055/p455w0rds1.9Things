...
public class BlockCompressor extends Block implements ITileEntityProvider {
	...
	private TileEntityCompressor getTE(World worldIn, BlockPos pos) {
		return (TileEntityCompressor) BlockUtils.getTE(worldIn, pos);
	}
	...
	public boolean onBlockActivated(World worldIn, BlockPos pos, IBlockState state, EntityPlayer playerIn, EnumHand hand, ItemStack heldItem, EnumFacing side, float hitX, float hitY, float hitZ) {
		if (!worldIn.isRemote) {
			TileEntityCompressor te = getTE(worldIn, pos);
			if (te != null && !playerIn.isSneaking()) {
				GuiHandler.launchGui(Globals.GUINUM_COMPRESSOR, playerIn, worldIn, pos.getX(), pos.getY(), pos.getZ());
			}
